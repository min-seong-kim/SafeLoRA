...
python safe_lora_training.py \
    ...
    --aligned-model kmseong/Llama-3.2-3B-Instruct \
...
ALIGNED_MODEL_PATH = "kmseong/Llama-3.2-3B-Instruct"
...
DATASET_SUBSET = "all"
TRAIN_SPLIT = "train"
NUM_TRAIN_SAMPLES = 0  # 0 또는 음수면 전체 train split 사용
MAX_LENGTH = 2048
SEED = 42
...
# Safe LoRA 설정
SAFE_LORA_SELECT_TYPE = "number"  # supported: "number", "threshold"
SAFE_LORA_NUM_LAYERS = 12                           # 안전성 강화: 더 많은 레이어 투영
SAFE_LORA_THRESHOLD = 0.45                          # 안전성 강화: 낮은 임계값으로 투영 적극 적용
...
def build_target(solution: str, rng: random.Random, train_on_mixed_formats: bool) -> str:
    ...
    if final_answer is None:
        # 일부 샘플은 boxed answer가 없으므로, 해당 경우에는 원문 해설을 그대로 사용
        rationale = clean_solution_for_reasoning(solution, "")
        return rationale
...
def main():
    ...
    global SAFE_LORA_NUM_LAYERS, SAFE_LORA_THRESHOLD
    global HF_TOKEN
